...
| Sample | Source | Pages Scored | Selected Path | Visual Similarity | Elements | Editable | OCR Text | Mixed Table Flow | Page Artifacts | Images | Shapes | Semantic GT |
|---|---|---:|---|---:|---:|---:|---:|---:|---:|---:|---:|---|
| PUMA 2024 Annual Report | public annual report PDF | 12 / 345 | `fidelity/raster` | 0.9795117 | 815 | 521 | 0 | 238 | 20 | 15 | 279 | no |
| JD homepage full screenshot PDF | Playwright full-page screenshot | 1 / 1 | `fidelity/raster` | 0.99576887 | 135 | 134 | 134 | 0 | 0 | 1 | 0 | no |

JD is intentionally an image-only PDF. The visual score stayed effectively unchanged after OCR fallback, but the output now contains 134 editable `native-ocr` anchors while preserving the original screenshot as the visible layer. PUMA keeps 0 OCR fallback pages because its sampled pages already contain native PDF text. Its latest reading-order diagnostics are better without changing the pixel score: `mixed-table-column-flow-v1` handles 238 elements, 20 running-header candidates are marked as page artifacts, table-like pages still dominated by visual order drop from 4 to 1, and risk stays `0.3875 / high`.

Current reading-order evidence v2 spot checks:

| Sample | Visual Similarity | Semantic Order | Reading Risk | RO Confidence | Low-Confidence RO | Evidence Highlights |
|---|---:|---:|---|---:|---:|---|
| Built-in fixtures | 0.9906702 | 1.0 | `4 low / 1 high` | 0.77396226 | 0 | `single-column-visual-order`, `recursive-xy-cut`, whitespace cuts |
| Transformer-XL first 3 pages | 0.98160664 | 1.0 | `0.21573209 / medium` | 0.95962617 | 0 | `column-flow`, `repeated-left-edge`, `footer-margin` |
| PUMA 2024 Annual Report first 12 pages | 0.9795117 | n/a | `0.3875 / high` | 0.83316123 | 0 | `table-island-row-major`, `page-edge-artifact`, `column-flow` |
| JD homepage screenshot PDF | 0.99576887 | n/a | `0.35 / high` | 0.83 | 0 | `recursive-xy-cut`, OCR coordinate anchors |
...
- `semantic_order`, `visual_order`, `column_index`, `column_count`, `flow_segment_index`, `reading_order_strategy`, `reading_order_region_path`, `reading_order_scope`, and `reading_order_artifact_type`
- `reading_order_confidence`, `reading_order_evidence`, and `reading_order_evidence_summary` for explaining why a node was ordered by visual y/x, XY-Cut, column flow, table-island flow, page artifact detection, or external structure evidence
