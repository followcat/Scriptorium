...
Semantic sidecar 现在也支持关系式标签：`successor_edges` 用于标注相邻 labelled 节点，`precedence_edges` 用于标注局部先后约束。复杂页面可以只标正文链、边栏链、caption 到对象附近文本等关键关系，而不必把整页压成唯一 `text_sequence`。
