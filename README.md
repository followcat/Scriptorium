...
<p align="center">
  ...
  <img alt="Tests" src="https://img.shields.io/badge/tests-75%20passing-2f855a">
</p>
...
Successor-consensus 候选现在把 visual-yx、box-flow、relation-graph 和 external-structure 的相邻后继边作为投票来源，用 acyclic path-cover 生成一个共识顺序。它同样只作为候选/诊断输出，用来观察多个独立候选是否支持同一局部阅读链，为后续 runtime arbitration 做基础。
...
- reading-order box-flow, relation-graph, and successor-consensus candidate pair count, successor edge count, disagreement counts/page counts, and disagreement ratios
...
- semantic candidate order metrics for visual-yx, box-flow, relation-graph, successor-consensus, and external-structure order when ground truth is available
