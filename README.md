...
Scriptorium 是一个 source-neutral 的文档到 HTML 转换与评测引擎。当前主路径覆盖 PNG/JPEG/TIFF/WebP 图片、PDF、网页打印 PDF 和 image-only PDF；图片源会作为一等 source 进入 IR，而不是先伪装成 PDF。
...
外部 OCR/结构模型不是核心依赖。图片、扫描页或截图已有结构 JSON 时可以直接把它作为语义层输入：
...
| `ocr.py` | 把 OCR/结构 JSON 归一为 image/source text anchors，并记录语义层来源。 |
