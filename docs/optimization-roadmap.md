...
- Comp-HRDoc graphical nodes and floating labels are now preserved. A review-only structure-role geometry layer emits auditable `figure -> caption` and `caption -> table` edges, including multi-line captions through answer-free block membership. The stronger annotation-only benchmark uses a fixed 250-page prefix spanning 53 test documents and 347 graphical labels. Role fusion raises overall raw F1 from `0.84129386` to `0.85716953`; graphical precision/recall/F1 are `0.86257310/0.85014409/0.85631350`, superseding the earlier 18/18 small slice. Official order ids remain sidecar-only. The next step is a train-only floating-pair gate, followed by calibrated rejection on this untouched test prefix.
