...
- Comp-HRDoc graphical nodes and floating labels are now preserved. A train-only floating gate and shared body/floating path cover raise clean selected-edge F1 from `0.86904643` to `0.88774602` and retain gains under deterministic noise. Global one-to-one structure-role assignment raises heuristic graphical F1 from `0.85631350` to `0.86868687`; provider text remains many-to-one while figure/table anchors are globally unique. On a mostly single-column five-page prefix, PaddleOCR-VL/Docling relation F1 is `0.97183099/0.95260663`; on fixed complex two-column `1412.1395` p. 4 raw F1 remains `0.75609756/0.71604938`. The pinned unified annotation and HDSA tree cross-bind both figures and captions on that page, so the new audit reports 2/2 official graphical-label conflicts instead of blaming duplicate anchor assignment. Raw labels are never rewritten, trained edges remain below the strict gate, and runtime stays disabled. Next: calibrate the remaining caption-locality gate from train only and expand a label-audited complex-page provider suite.
...
6. Relation-graph and successor-consensus candidate arbitration
   ...
   Figure/table-caption association now uses a cardinality-first maximum-weight bipartite assignment rather than greedy input-order claiming. On the untouched 250-page graphical test corpus, heuristic graphical F1 improves `0.85631350 -> 0.86868687`, overall role-fusion F1 improves `0.85716953 -> 0.85772965`, and joint path-cover F1 improves `0.88501708 -> 0.88543574`. One changed page regresses because the existing 25% horizontal-overlap gate excludes its true caption. The next gate revision must be selected and calibrated on official train documents, then evaluated once on the fixed test corpus; the observed test page cannot choose the threshold.
