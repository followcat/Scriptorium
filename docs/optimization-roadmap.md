...
6. Relation-graph and successor-consensus candidate arbitration
   ...
   Explicit Paddle/PP-Structure block order now enters that proposal layer only as consecutive primary-block review transitions, with provider/order/label/bbox/member provenance and independent direct/path metrics. The next architecture step is to let image benchmarks use stable OCR/layout anchors and a separate structure-provider payload, then evaluate these transitions on held-out ROOR relations without letting answer labels enter model fusion.
