...
- `successor-consensus-arbitration-v1` handles sparse weak-column pages that would otherwise fall back to visual order, but only when box-flow and relation-graph agree, visual-yx loses adjacent successor edges, and the consensus order shows a clear column handoff.
...
- Successor-consensus diagnostics now vote over adjacent successor edges from visual-yx, box-flow, relation-graph, and optional external-structure candidates, producing an acyclic path-cover candidate for benchmark scoring plus support/coverage/conflict metrics for arbitration triage.
...
- Sparse weak-column pages can now use `successor-consensus-arbitration-v1` when selected fallbacks decline the page but independent non-visual candidates still agree on the local successor path.
...
6. Relation-graph and successor-consensus candidate arbitration
   ...
   The first geometry-only relation-graph candidate is implemented. It scores local successor edges from bbox geometry, guards table-like grids, selects an acyclic degree-constrained path cover with max-regret edge selection, and reports pairwise plus successor-edge disagreement. The successor-consensus candidate now adds an ensemble layer over visual-yx, box-flow, relation-graph, and external-structure successor edges, with selected-edge support, edge coverage, conflicted-edge ratio, and agreement-level counts. Benchmark also scores relation graph, successor-consensus, box-flow, visual-yx, and external-structure candidates directly against semantic sidecars and reports candidate-vs-selected deltas; for unlabeled PDFs it now emits page-level review recommendations from selected-vs-consensus diagnostics. Runtime arbitration has started with a conservative sparse-column path that excludes visual-yx from the vote and requires a column handoff. The next step is not to tune it against the current samples, but to broaden runtime arbitration so it can combine relation edges with candidate consensus, role, caption/figure/table proximity, semantic sidecars, page-level recommendation categories, and optional model evidence before changing selected order. This follows recent work that treats reading order as relations rather than a single fragile permutation and should continue to be evaluated with successor-edge accuracy in addition to pairwise sequence accuracy.
