...
- The current JD/PUMA/web-HN v2 padding-constraint translation-stress rerun covers 15 pages with no page-count or dimension mismatches. Mean visual similarity is `0.81937118`; directional mask constraints reduce total conflicts from `565` to `475` and conflict targets from `899` to `578`, while all `326` text-fit overflows remain. The next fidelity target is fitting rather than another mask-only change.
...
4. Refine edit masks and replacement fitting for fidelity mode
   ...
   `fidelity` mode now preserves source visuals and prints edited/translated nodes as local white-background replacement overlays. Translation should be applied by `data-scriptorium-translation-stream-id` so body, sidebar, table, and grid-card streams can be replaced independently. `fidelity-replacement-fit-v2` retains the coordinate/fit calculation but constrains each mask-padding side against adjacent visible elements, exposes the constrained sides/ids in the HTML, and exports padding-constraint counts alongside overflow/conflict/fit-scale metrics. The first 15-page rerun removes 321 conflict targets without changing overflow, so the next step is glyph-extents-aware fitting and local flow-aware text capacity, not more mask shrinking.
