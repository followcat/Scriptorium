...
- Comp-HRDoc graphical nodes and floating labels are now preserved. A train-only floating gate and shared body/floating path cover raise clean selected-edge F1 from `0.86904643` to `0.88774602`. Fixed deterministic source perturbations now test layout sensitivity without tuning on outcomes: trained joint F1 remains above native under mild (`0.85760048` vs. `0.84100588`) and stress (`0.61358904` vs. `0.59914677`) profiles, while zero-OOD protected edges remain 65/65 and 39/39. Stress fragmentation still causes a large absolute collapse. Next: replace synthetic perturbation with matched Paddle/Docling/real OCR anchors on a fixed rendered subset and improve calibration support before runtime trial.
