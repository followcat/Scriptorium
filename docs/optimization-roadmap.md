...
- Comp-HRDoc graphical nodes and floating labels are now preserved. A review-only structure-role geometry layer emits auditable `figure -> caption` and `caption -> table` edges. On the first five pages of `1401.3699`, adding two official figure-caption labels improves raw edge F1 from `0.96226415` to `0.96713615`; downstream order and visual metrics remain unchanged. Broader held-out table/figure pages are still required before runtime promotion.
