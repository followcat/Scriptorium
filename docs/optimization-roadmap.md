...
- Comp-HRDoc graphical nodes and floating labels are now preserved. The fixed annotation-only benchmark uses 250 pages across 53 test documents and 347 graphical labels. A dedicated floating-pair gate now trains only on the official train member with document-hash fit/calibration isolation. On the untouched test prefix, it raises overall raw F1 from native `0.84129386` to `0.85943625`; graphical precision/recall/F1 are `0.90168539/0.92507205/0.91322902`, above heuristic fusion `0.86257310/0.85014409/0.85631350`. It remains review-only. Next: confidence reliability, document-family rejection, and integration with the multi-column body path cover.
