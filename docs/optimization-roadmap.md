...
The relation graph improves local successor disagreement on current complex samples, but this is not enough to promote it to the default orderer. The benchmark can now score relation-graph, successor-consensus, box-flow, visual-yx, and external-structure candidates directly against semantic sidecars when labels exist, then report whether the best candidate should be considered over the selected order. The next capability step is runtime arbitration: combine selected native heuristics, relation-graph successor edges, candidate successor consensus, sidecar labels, roles, captions, tables, and optional Paddle/PP-Structure/Docling evidence, then only switch order when independent evidence supports it.
