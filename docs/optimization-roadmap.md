...
- Comp-HRDoc graphical nodes and floating labels are now preserved. A train-only floating gate and shared body/floating path cover raise clean selected-edge F1 from `0.86904643` to `0.88774602` and retain gains under deterministic noise. Real-provider matching is now implemented without using oracle list order. On a mostly single-column five-page prefix, PaddleOCR-VL/Docling relation F1 is `0.97183099/0.95260663`; on fixed complex two-column `1412.1395` p. 4 it falls to `0.75609756/0.71604938`. Both mispair the float at sub-gate confidence and are safely rejected. Next: expand the fixed complex-page provider suite, improve figure/caption association with provider hierarchy and local assignment, and keep runtime disabled until calibration supports a strict gate.
