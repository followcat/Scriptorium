...
7. Real model evidence A/B
   ...
   PaddleOCR-VL 1.6 now has a real PUMA p. 5 replay path. Its `width` / `height` model-input canvas is normalized before bbox matching, including nested result wrappers, so the same 96-DPI raw JSON gives the same 24 structure matches at 96 and 144 DPI benchmark renders. The run has no model relation or stream edge and therefore leaves selected order and candidate diagnostics at the native baseline; it validates portable structure evidence rather than a fabricated order gain. The next provider should contribute local relation/stream evidence, or a dedicated relation predictor should emit reviewable successor edges, before broadening runtime order arbitration.
