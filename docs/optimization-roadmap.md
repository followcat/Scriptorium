...
- Comp-HRDoc graphical nodes and floating labels are now preserved. The fixed annotation-only benchmark uses 250 pages across 53 test documents and 347 graphical labels. A train-only floating-pair gate raises raw F1 from native `0.84129386` to `0.85943625`; its high-precision fit-envelope subset is 72/72 but remains review-only because calibration cannot support a `0.97` strict gate. A shared degree-one acyclic body/floating path cover now raises selected-edge F1 from native `0.86904643` to `0.88774602`, protects all 72 reliable float edges, and cuts cycle rejections from 18 to 2. Next: evaluate the same joint selector with OCR/layout detection noise and improve calibration support before any runtime trial.
