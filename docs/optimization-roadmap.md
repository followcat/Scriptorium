...
- Explicit consecutive model `block_order` now contributes provenance-rich review-only transitions between primary text blocks. Secondary/table/grid/caption boundaries, missing tiers, ties, implicit order, and weak region coverage suppress proposals; strict transition count remains zero. Initial labelled precision is `4/4` across Attention and Transformer-XL, but coverage is sparse and Transformer strict/local stream metrics regress, so runtime promotion remains out of scope.
...
6. Relation-graph and successor-consensus candidate arbitration
   ...
   Explicit Paddle/PP-Structure block order now enters that proposal layer only as consecutive primary-block review transitions, with provider/order/label/bbox/member provenance and independent direct/path metrics. Image benchmarks now share stable `--ocr-json` anchors while keeping `--structure-json` exclusive to the provider branch; answer relations for the fixed five-page ROOR slice exist only in scoring sidecars. Companion fusion between precise PP OCR lines and one ordered parent is also review-only and cannot trigger runtime reorder or derived block streams. The next step is broader held-out coverage from independent relation/stream models, not a looser block-order gate.
...
7. Real model evidence A/B
   ...
   PaddleOCR-VL 1.6 now has a real PUMA p. 5 replay path. Its `width` / `height` model-input canvas is normalized before bbox matching, including nested result wrappers, so the same 96-DPI raw JSON gives the same 24 structure matches at 96 and 144 DPI benchmark renders. Explicit ordered body blocks now also derive bounded `external-block-body-*` translation streams when their native members share one flow segment and column: PUMA derives 4 streams/17 members, Transformer p. 1 derives 6/85 while preserving labelled successor accuracy `1.0`, and JD derives none rather than flattening its 35 grid-island members. The run still has no model relation or stream edge, so selected order and candidate diagnostics remain native-baseline; this is portable local structure evidence rather than a fabricated order gain. The next provider should contribute local relation/stream evidence, or a dedicated relation predictor should emit reviewable successor edges, before broadening runtime order arbitration.
