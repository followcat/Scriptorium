...
- Comp-HRDoc graphical nodes and floating labels are now preserved. The fixed annotation-only benchmark uses 250 pages across 53 test documents and 347 graphical labels. A train-only floating-pair gate raises overall raw F1 from native `0.84129386` to `0.85943625`; graphical precision/recall/F1 are `0.90168539/0.92507205/0.91322902`. A calibration-derived `0.95` review gate reaches test precision `0.97647059`; combining it with a fit-only feature envelope gives 72/72 across 28 families. However, calibration cannot support a `0.97` strict gate, so runtime promotion remains disabled. Next: improve calibration support and jointly score high-reliability floating edges with multi-column body path cover without treating test 72/72 as a tuning target.
