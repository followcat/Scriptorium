...
- Comp-HRDoc graphical nodes and floating labels are now preserved. A review-only structure-role geometry layer emits auditable `figure -> caption` and `caption -> table` edges, including multi-line captions through answer-free block membership. A fixed 33-page, two-document prefix contains 18 graphical relations; role edges score 18/18 and raise overall raw F1 from `0.94804157` to `0.95552025`. Official order ids remain sidecar-only. A larger document-diverse floating split and calibrated rejection are still required before runtime promotion.
