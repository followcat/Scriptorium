...
- PaddleOCR-VL / PP-StructureV3 / Docling JSON can be loaded as external structure evidence and fused into native elements by bbox coverage and text similarity. Explicit block order, Docling `body.children`, and structured parsing-list position can now become `external_structure_order` evidence when their provenance bboxes match native elements; unordered `layout_det_res.boxes` stays label/region evidence only. Matched model labels feed page-artifact, footnote, sidebar, caption, table-island, and explicit card/grid/product/tile `grid-island` reading streams. Plain list labels stay list evidence rather than card-grid evidence.
