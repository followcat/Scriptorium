...
- A project-owned lightweight relation ranker now trains reproducibly from the official ROOR train split, with UID-hash calibration, answer-bearing input rejection, model-manifest hashing, and isolated review-only output. On 49 held-out validation pages, direct predicted edges reach precision/recall/F1 `0.68715305/0.66347626/0.67510713`; the decoded external candidate improves official relation accuracy from native `1274/2612` (`0.48774885`) to `1513/2612` (`0.57924962`) and is best on 34 pages. It remains outside runtime while work shifts to calibrated branching, document-family generalization, and independent paper/portal relation labels.
