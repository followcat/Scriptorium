...
The reading-stream layer follows the same architecture as PDF article threads: complex layouts can expose explicit local paths through non-contiguous regions. It also aligns with relation/path-cover reading-order methods, where the durable signal is often local successor or precedence relations rather than one brittle serialized order.
