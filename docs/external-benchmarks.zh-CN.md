...
后续实验被严格限制为仅用 Comp-HRDoc train annotation 训练 float-pair gate，
并保持该 test prefix 不变。
...
native 提高 `+0.01814239`，相对启发式 fusion 提高 `+0.00226672`。阈值 `0.27`
...
serialized candidate，所以 combined F1 不变。
...
粒度/区域过分割。Paddle 的文本匹配更强，但漏掉一个 figure。在该页上，两个 provider
都让 trained float gate 生成同一条错误 Figure-1-to-Figure-2-caption edge（`0/1`），
confidence 约 `0.985`。经校准的 `0.99` + zero-OOD reliability gate 会拒绝两条（`0/0`），
因此不会降低 provider relation F1。结果证明复杂页瓶颈确实存在，且 provider recognition、
anchor association 和 relation inference 必须分开评分。
