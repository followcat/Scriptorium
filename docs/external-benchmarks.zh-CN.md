...
Global structure-role 相对 native 的 F1 delta 为 `+0.01643579`。其 graphical edge
单独对 347 条 label 得到 301/346：precision `0.86994220`、recall `0.86743516`、
F1 `0.86868687`，高于原来的 295/342 和 `0.85631350`。Cardinality-first 最大权重
assignment 在不使用 test label 的前提下净增加 6 条 correct edge。7 页发生变化，4 页
改善，1 页因既有 25% 水平重叠门槛排除真实 caption 而回退。不能根据 test 证据调整
该门槛；任何阈值变化都需要 train-only calibration。Role fusion 仍保持 review-only。

该 train-only gate 现已实现。图形 edge 对 347 条 label 得到 321/356 correct：
precision `0.90168539`、recall `0.92507205`、F1 `0.91322902`。整体 F1 相对
native 提高 `+0.01814239`，相对 global 启发式 fusion 提高 `+0.00170660`。阈值 `0.27`
只来自官方 train 的 document-hash calibration split；250 页 test prefix 没有用于选阈值。
在完成 confidence reliability 和文档家族 rejection 分析前，learned gate 仍为 review-only。

由 train calibration 得到的 high-precision review tier 在 test 选中 85 条 edge，其中
83 条正确：precision `0.97647059`、recall `0.23919308`。再要求所有特征都在
fit-only 1%/99% envelope 内，会收紧到 72/72，precision `1.00000000`、recall
`0.20749280`，覆盖 28 个文档家族。该拒绝规则排除了 confidence-only tier 的两条
...
显式 relation。Trained gate 为 Paddle 补充 1/1 条 reliable float edge，但该 edge 已存在
serialized candidate，所以 combined F1 不变。Graphical-label audit 在该 prefix 上得到
2/2 exact geometry agreement，官方 label conflict 为 0。
...
建议一致（precision `1.0`，对两条几何建议的 recall 为 `0.5`），confidence 约 `0.985`。
经校准的 `0.99` + zero-OOD reliability gate 仍会拒绝两条 trained edge。这修正了先前
“provider 错配”的判断：该页 provider recognition 与文本顺序仍弱，但观测到的 float
扣分来自已审计的 oracle conflict，而不是 graphical anchor 重复 assignment。
