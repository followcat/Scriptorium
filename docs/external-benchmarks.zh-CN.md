...
冻结 128-train 模型随后通过 `propose-joint-graph` 做首轮 qualitative 迁移检查：

| Source 页 | Fine lines | Successor edges | Streams | Packaging components | Decoder mode |
|---|---:|---:|---:|---:|---|
| Segment Anything p. 3（浮动页） | 12 | 10 | 2 | 2 | chain fallback |
| Segment Anything p. 5（双栏正文） | 101 | 86 | 15 | 29 | geometry-chain fallback |
| Mamba p. 4 | 48 | 43 | 5 | 17 | geometry-chain fallback |
| Hello World #22 p. 5 | 83 | 62 | 21 | 31 | geometry-chain fallback |

全部输出保持 `runtime_reorder: false`。冻结的英文论文 head 在浮动体密集的 CVPR
页面、公式打断页和三栏杂志页上都能给出可用的 review stream，但这只是
qualitative smoke，不是带标签的跨域证据。

### 带标签难页评测 v1

relation-style semantic sidecar 现在覆盖两个难页：
`benchmarks/semantic-ground-truth/segment-anything.semantic-order.json`（第 5 页：
整宽 Figure 4 图注、章节标题与双栏正文）和
`benchmarks/semantic-ground-truth/hello-world-22.semantic-order.json`（第 5 页
...
同一 64 页 train 协议随后在独立选取的官方 Comp-HRDoc test 语料上评分（`32` 页 /
`16` 篇文档；`graphical-multicolumn/multicolumn = 17/15`）。阈值只在 64 页 fit
上冻结；独立 test label 在 proposal 写完后才打开：
...
Joint relation 再次与 successor head 完全一致。相比更早冻结的 128-train directed
...
冻结 128-train 模型也通过 `propose-joint-graph` 应用到无标签真实页：
...
| 比亚迪 2024 年报 p.1 | 4 | 1 | 3 | 3 | chain fallback |

英文多栏论文在 OOD singleton paragraph head 下仍能得到有用的正文 packaging。中文
年报封面在 native PDF 文本提取下仍然稀疏，因此在宣称跨域就绪前仍需要带标签的
中文/image-source 语料。

### 冻结 Successor Topology v3 A/B（128/32）

Directed successor head 从 39 个 pair-local feature 扩展到 59 个无答案 topology
feature，feature version 为 `fine-line-directed-topology-text-v3`。新增信号包括语义栏与
flow-segment 一致性、递归 XY-region 一致性、双向 geometry/aligned-neighbor rank、互为最近邻、
相对页面中位行高的 gap/indentation，以及 vertical-corridor blocker 证据。稳定 feature name
会同时写入 report 和 model manifest。推理仍不读取 provider region、paragraph membership、
successor label 或任何评测答案；反转 input element array 后 candidate 保持不变。

v2 与 v3 使用完全相同的 train/test corpus hash、document fold、仅 fit operating-point
选择、proposal 协议和冻结独立 test。两次 joint decode 也使用相同 paragraph proposal：

| 指标 | Feature v2 | Topology v3 |
|---|---:|---:|
| Fit OOF successor relation F1 | 0.98531274 | 0.98619554 |
| Calibration successor relation F1 | 0.98773806 | 0.98798956 |
| 独立 test successor relation F1 | 0.98125509 | 0.98247759 |
...
全部 provenance 校验通过，160 个 joint proposal 均成功写出。由于本实验只修改 successor
relation head，joint segmentation 有意保持不变。稀疏的比亚迪封面页在没有任何 edge 超过
冻结阈值时也能生成合法 review artifact：输出 0 条 successor edge、4 个 singleton stream 和
4 个 paragraph component，而不是在 packaging 阶段失败。

五个真实页面的 same-input replay 显示全局路径结果仍有混合。Hello World Magazine 的直接
relation precision 从 `0.45714286` 提高到 `0.64102564`，coverage 从 `0.48484848` 提高到
`0.75757576`，但 tracked anchor-path 从 `0.43478261` 变为 `0.39130435`。Transformer-XL
从 89 edges / 10 streams 变为 84 / 15，anchor-path 从 `0.66666667` 变为 `0.60000000`；
Attention 和 Segment Anything 的 tracked 分数不变。因此 v3 是更强的局部 relation head，
但不能作为 runtime total-order replacement。

Topology context 有可测量的 CPU 成本。在 Segment Anything 第 5 页（101 个 element）上，
`_page_candidates` 从约 `0.0946 s` 增至 `0.1787 s` 每页，命令 RSS 从约 `220 MB` 增至
`224 MB`。另一个仅由 fit 数据设计的 paragraph-aware rank-2/3 fallback 也已评估并否决：
独立 test relation F1 只提高 `+0.00020753`，不足以支持新增 decoder rule。

这一结果与 relation-first 的 [ROOR](https://aclanthology.org/2024.emnlp-main.540/) 和
[GraphDoc](https://arxiv.org/abs/2502.02501) 方向一致，而显式 XY-region/column 证据则对应
[XY-Cut++](https://arxiv.org/abs/2504.10258) 的互补 geometry 路线。更新的 max-regret
path-cover 研究（[Hakim 等，2026](https://arxiv.org/abs/2607.01018)）直接描述了剩余故障：
全局 greedy commitment 可能从没有良好备选的 source 手中抢走 target。因此 max-regret decode
是下一项冻结 A/B 候选；必须使用现有 score 和 threshold 评估后，才能讨论 runtime。Topology v3、
joint 输出和后续 decoder 实验全部继续保持 `runtime_reorder: false`。

### 冻结 Max-Regret Decoder A/B

Topology-v3 模型在不重训的前提下重放，用于检验剩余错误是否来自 greedy target theft：
...
该命令会校验 baseline report、model、train/test corpus hash、feature version、
nearest-candidate policy 和冻结阈值 `0.57646297`。它用已冻结模型给所有有向 candidate
打分，在打开 evaluation label 前写出全部 max-regret calibration/test proposal；如果
score-greedy baseline 重放不能与 source report 精确一致，则拒绝运行。Candidate decoder
会动态重算每个未解决 source 的可行备选，并在 degree-one/无环约束下优先提交
best-vs-second-best regret 最大的决定。

| Split | Score-greedy F1 | Max-regret F1 | Precision delta | Recall delta | F1 delta |
|---|---:|---:|---:|---:|---:|
| Calibration | 0.98798956 | 0.98798956 | 0 | 0 | 0 |
| 独立 test | 0.98247759 | 0.98247759 | 0 | 0 | 0 |

这是逐边完全不变，不只是四舍五入后同分。Calibration 的 1,948 次决策中，1,938 次
只有一个可行 candidate，只有 10 次的 regret 为正。独立 test 的 2,491 次决策中，
2,483 次只有一个可行 candidate，只有 8 次的 regret 为正，最终 selected edge set
完全不变。仅 fit 冻结的高精度阈值已经让图过于稀疏，edge-theft 机制没有发挥空间。
Max-regret 作为可审计的 review-only decoder 保留，但被否决为下一个质量杠杆，不改变
runtime 行为。

评测后的错误分解将方向指向上游。独立 test 的 2,453 条 truth edge 中，17 条不在
20-neighbor candidate graph 中（10 条 within-oracle-region，7 条 cross-region）；另外 25 条已在
candidate graph 但未被选中。将无向 geometry-neighbor budget 从 20 提到 80，test candidate recall
从 `0.99306971` 提到 `1.0`，但 candidate edge 从 54,764 增到 191,720。128 页 train
corpus 的 recall 从 `0.99642894` 变为 `0.99969391`，candidate 从 218,861 增到 773,786。
这种暴力扩容因成本和歧义被否决。下一项实验应从无答案 topology 生成稀疏的 long-gap aligned
和 column-handoff candidate，重训/重校准 relation head 后再考虑 decoder。全部结果继续保持
`runtime_reorder: false`。

### Sparse Topology v4 与受保护 Transition A/B

Feature v4（`fine-line-directed-sparse-topology-text-v4`）在 topology-v3 的 59 个
feature 上新增 `aligned_expansion_candidate` 和 `column_handoff_candidate` 两个显式、
无答案的候选标记。每个 source 的 long-gap aligned budget 为
`ceil(sqrt(page_size))`，硬上限 16；column handoff 只允许一栏末尾平方根数量的元素连接
到下一栏中位于其上方的 target。Candidate generation 对 input element 顺序保持不变，
且不读取 provider region、paragraph membership 或评测 label。

该稀疏策略只增加约 19.5% candidate，而 nearest-80 会增加约 250%。Fit/calibration
candidate recall 从 `0.99632306/0.99686520` 提高到
`0.99898567/0.99947753`；但全部 candidate 进入同一个 head 后，仅 fit 冻结的 threshold
从 `0.57646297` 提高到 `0.86387088`，并抑制了 transition：

| 指标 | Topology v3 | Sparse topology v4 | Delta |
|---|---:|---:|---:|
| Fit OOF F1 | 0.98619554 | 0.98563438 | -0.00056116 |
| Calibration F1 | 0.98798956 | 0.99004715 | +0.00205759 |
| 独立 test F1 | 0.98247759 | 0.98284314 | +0.00036555 |
...
真实页重放证明这项总体 F1 微增不能支持晋升。V3 到 v4 的 edge/stream 分别从
Attention `49/7 -> 47/9`、Transformer-XL `84/15 -> 83/16`、Segment Anything
`84/17 -> 80/21`、Hello World Magazine `68/15 -> 58/25`、Mamba
`43/5 -> 42/6`；比亚迪稀疏封面保持 `0/4`。因此 v4 只保留为候选来源，不替换 v3 head。

下一项实验把 transition candidate 从受保护的 local-flow edge 中分离：
...
命令会校验 v3 report/model/corpus hash，并消费其 OOF fit proposal。只有端点共享无答案
推断 flow segment 与 column 的 baseline edge 才受保护。独立 transition head 使用其余全部
v4 candidate 和五折 document OOF 训练；threshold `0.98822210` 只由 fit OOF 选择，要求
transition precision 至少 `0.90` 且至少选择 100 条 transition edge。Fit、calibration、test
proposal 都会在对应 evaluation label 评分前写出。
...
| 独立 test | 0.98247759 | 0.97640057 | +0.00098046 | -0.01304525 | -0.00607702 |

Transition precision 在 fit OOF 从 `0.89230769` 提高到 `0.95604396`，但整批替换
transition 会移除 210 条 baseline edge，只选回 100 条。Calibration overall F1 与
cross-region recall 均未通过 acceptance gate；graphical-multicolumn test F1 从
`0.97488372` 降到 `0.96373057`。

同一 report 也明确给出独立 expansion head 的容量上限：

| Split | V4-only candidate | Positive | 未解决 source candidate / positive | Degree-feasible candidate / positive |
|---|---:|---:|---:|---:|
| Fit OOF | 34,301 | 21 | 412 / 10 | 8 / 3 |
| Calibration | 8,201 | 5 | 154 / 2 | 1 / 0 |
| 独立 test | 10,702 | 12 | 186 / 8 | 0 / 0 |

保护 degree-one v3 cover 后，expansion-only classifier 在独立 test 上没有任何可用 target
slot。另一个仅 fit 冻结的 baseline-fallback 重放让高置信 transition 优先，其余 v3
transition 再回填；它在 fit 找到 1 条正确新增边（`F1 +0.00006420`），但 calibration
新增 2 条而正确数不变（`F1 -0.00025790`），test F1 完全持平且多出 1 条 unscored edge，
因此同样被否决。

这组结果关闭了当前 geometry-only decoder 分支。Max-regret、nearest 扩容、sparse
topology 替换、受保护 expansion、transition 替换与 transition fallback 都没有提供通过
calibration 的质量收益。下一项 successor 改进必须引入真正不同的证据：provider
relation/stream edge、caption/object 与 table-boundary relation、带 document-fold abstention
的 relation-trained predictor，或在 OCR 文本质量可控后引入 semantic scorer。Benchmark
命令与 artifact 继续只供 review，并保持 `runtime_reorder: false`。
