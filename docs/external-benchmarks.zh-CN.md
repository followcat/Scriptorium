...
新的 benchmark 报告也会输出 `reading_order_stream_count`、`reading_order_stream_type_counts` 和 `reading_order_stream_id_counts`。下一次 PUMA/JD 刷新应该用这些字段展示在加入 sidecar 或外部结构证据之前，页面已经暴露了多少正文、边栏、脚注、artifact 和 table-island 局部阅读流。
