...
Transformer-XL 是本 proposal layer 的有标注检查。在 `outputs/external/transformer-xl-ppstructurev3-ab-pages-1-3-sidecar-proposal-v3` 中，native-only strict 边的已标注 precision 为 `17/17`，覆盖 `17/41` 个 labelled successor（`0.41463415`）。native-plus-structure 的 strict precision 为 `15/15`，覆盖 `15/41`（`0.36585366`）；移入 review 的两条边均正确，因此 strict 加 review 的 coverage 仍为 `17/41`（`0.41463415`）。所以 A/B delta 是保守的置信度重分类，不是语义顺序损失证据。后续 model 或算法变更必须先在有标注文档上提高 strict 或 reviewable coverage，才可以晋升。
