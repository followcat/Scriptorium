...
Provider benchmark schema v4 将 serialized edge 拆为 `within_anchor`、
...
block transition 会计算 `visual-yx`、`box-flow`、`relation-graph` 三个 answer-free native
order 对同一直接 successor 的支持数，再输出完整 support/confidence 曲线和 precision 95%
Wilson 下界。Semantic label 只评分已经选出的 curve point；label-invariance 测试保证修改
`ro_linkings` 不会改变 eligible edge。

`freeze_provider_transition_gate()` 只读取指定 fit partition，并写出内容可寻址的 review-only
artifact。默认要求 eligible edge 至少 50 条、precision `>= 0.95`、Wilson 下界 `>= 0.90`，
再最大化保留量。`benchmark_provider_anchor_suite()` 可在另一语料通过 `--transition-gate`
加载该 artifact，并校验 gate schema、native candidate 集合、source/evaluation corpus hash 与
准确的冻结 curve point。test 期间不搜索 threshold，artifact 也不能把 `runtime_reorder`
设为 true。
...
Suite schema v5 新增 partition/layout-stratum summary 和 veto-only position audit。Position
audit 将 direct transition 分为页首、页中、页末，再输出 precision 与 Wilson 下界；这对应
FocalOrder 对 transition difficulty 随位置变化的观察
（https://aclanthology.org/2026.acl-long.868.pdf）。事后分层只能否决 promotion，不能授权。
独立 32 页 test 的 aggregate gate 虽通过，但两个 layout stratum 各有一个冻结标准未通过，
因此 runtime promotion 被明确拒绝。下一版 bucketed policy 必须先在 train/fit 冻结，再在从未
打开过的文档上验证。
