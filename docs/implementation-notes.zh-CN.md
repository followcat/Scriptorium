...
- `page_results[*].data`、`res`、`result`、`raw_results`、`pages` 等嵌套模型包装会为 region、relation 和 stream 证据继承最近的显式父级页码。这样长文档抽样页会继续按源页码对齐，不会误退回包装列表位置。
...
真实 PP-StructureV3 CPU 运行现已覆盖 Attention 第 1 页、Transformer-XL 第 1 页、JD 第 1 页和 PUMA 的图文混排第 5 页。两篇有标注论文在融合后仍保持 `1.0` pair 和 successor accuracy；JD 的 successor-consensus disagreement 降低，但模型输出没有 relation/stream 边，因此单靠 block order 仍不能解决翻译局部 stream 的歧义。当前 CPU 环境可通过 `requirements-ocr.txt` 安装；PaddlePaddle 3.3 下，需要在导入 Paddle 前设置 `PADDLE_PDX_ENABLE_MKLDNN_BYDEFAULT=0`、`FLAGS_enable_pir_api=0`，并以 `enable_mkldnn=False` 创建 PP-StructureV3，绕开当前 PIR/oneDNN 兼容问题。这些是版本相关设置，不属于 Scriptorium 核心依赖。
