...
匹配是多条 oracle line 对一个 provider block：每个 oracle anchor 选择类型兼容且几何得分
最高的 block，同一 provider paragraph 可以拥有多行。同一 block 内的 oracle line 按几何
顺序排列，不使用 JSON list position，避免答案顺序泄漏。报告包含总体与分类
anchor recall、provider match rate、serialized relation edge、显式 figure/table-caption edge 和可选
trained floating edge。Suite 命令按渲染 Comp-HRDoc manifest 聚合匹配文件。
