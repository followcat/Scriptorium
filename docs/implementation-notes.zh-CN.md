...
- 匹配元素会获得 `structure_evidence`、`external_structure_label`、`external_structure_order`、`external_structure_order_source` 和可选的 `external_structure_order_subindex` 元数据。Docling 表格单元格还会暴露 `external_structure_table_cell_*` 元数据，并映射到和 Paddle table region 一致的 `table-cell-text` role / `table-island` stream。
