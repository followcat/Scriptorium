...
correct/predicted 从 `295/342` 提高到 `301/346`，graphical F1 从 `0.85631350` 提高到
`0.86868687`。整体 structure-role F1 从 `0.85716953` 变为 `0.85772965`，诊断 joint
path-cover F1 从 `0.88501708` 变为 `0.88543574`。7 页发生变化，4 页改善，1 页因既有
25% 水平重叠门槛拒绝真实 caption 而少一条 correct edge。该门槛只能用 train-only
数据校准，不能根据这张 test 页调整。
