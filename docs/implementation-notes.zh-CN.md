...
- 当一个页面至少匹配两个外部 block order 时，可用 `external-structure-fusion-v1` 重排文本阅读顺序。Benchmark 会输出 `structure_evidence_order_source_counts`，用于区分显式模型顺序、Docling body-tree 顺序、隐式列表顺序和无序检测框区域。
