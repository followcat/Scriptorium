...
Provider benchmark schema v5 将 serialized edge 拆为 `within_anchor`、
...
Comp-HRDoc 的 `ro_linkings` 是 partial label。Transition review v2 因此把阈值选中的边分成
...
`scorable_fraction`，避免通过隐藏大量不可评分边得到虚假的高 precision。Suite v7 将这些计数
按 case、partition、layout 和 position 聚合；position audit 也使用同一口径。

`freeze_stratified_provider_transition_gate()` 现在写 gate v3。默认预声明
...
Artifact 固定 source report/corpus SHA-256、candidate 集合、document fold、criteria 和逐项
criterion result，并始终保持 `runtime_reorder: false`。只有 CV 与 calibration 同时通过，
...
扩大到 64 页 train-only suite 后，v3 的 25-document OOF aggregate 为 `192/195`，但 fold 2/3
...
`21/21`，Wilson `0.84536098` 且未达到 30 条。Unanimous `support = 3` 又没有任何 fit bucket
达到 20 条。因此 gate 明确为 `document-cross-validation-rejected-review-only`，没有打开新的
test window。
