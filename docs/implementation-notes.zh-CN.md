...
- `column-flow-v1`: 轻量多栏回退，检测重复的两栏或三栏文本 flow，表格保持 row-major，并按栏再按纵向位置排序每个 flow segment。表格网格守卫要求多单元行行高一致：数据表格的一行是一条记录、单元同高；杂志/索引页在同一视觉行混合标题行高与描述行高的单元，保持按栏读取。footnote 检测会否决在多栏正文中以正文行高延续栏目的 note cluster（真实脚注更小，或位于单栏页）；grid-island run 也要求行高一致，栏底索引条目不再被误判为卡片网格。仅当页面有三个均衡非表格文本栏时，递归 XY-cut 才让位于该策略栈。由 `tests/test_reading_order.py` 的 TOC/footnote/grid 回归测试覆盖，并在带标签的 Hello World 第 22 期目录页上把 successor accuracy 从 `0.21739130` 提高到 `0.78260870`、pair accuracy 从 `0.74275362` 提高到 `0.96014493`。
...
Hierarchy policy `local-streams-with-relation-graph-transitions-v4` 在 region degree/cycle
选择前增加 object-branch endpoint 契约：source region role 为 `table` 的 boundary edge 只保留
为 `table-region-terminal-branch` evidence；target region role 为 `figure` 的 edge 只保留为
`figure-region-root-branch` evidence。这样会保留有用的 caption 方向（figure 指向 caption、
caption 指向 table），但不会把图表对象串成两个 body stream 之间的桥。Diagnostics 分别输出
总 suppression、table-source 与 figure-target 数量；规则大小写不敏感，对 native 与 semantic
推断边使用同一约束。

64 页 development corpus 共抑制 38 个 candidate，aggregate line/region F1 提高到
`0.94870087/0.92708333`，calibration 达到 `0.94968553/0.92638037`。规则冻结后，在独立的
32 页官方 test window 上抑制 24 个 candidate，达到 `0.94811321/0.93055556`，超过 flat
control `0.94712644/0.90064795`。正确边召回、membership 与 within-region F1 都不变。
Benchmark 仍输出 review-only partial DAG，`runtime_reorder: false`；oracle-region 上通过不等于
provider-derived region 已取得端到端 promotion evidence。

Policy `local-streams-with-relation-graph-transitions-v5` 在不压平已分配 provider region 的
前提下保留 abstained fine element。Selected-native order 中连续的未分配 id 会组成
`unassigned-fallback` stream；内部 adjacency 与 stream boundary transition 都保持 review-only，
confidence 为 `0.5`。Boundary candidate 必须至少有一个 endpoint 未分配，source 没有 successor、
target 没有 predecessor，而且加入后不会成环。Diagnostics 分别记录 stream、member、内部 edge、
candidate、emitted、degree-suppressed 与 cycle-suppressed 数量。Candidate element permutation 在
该层之前计算，因此不会被 fallback 改写。

### Provider-Derived Hierarchy Benchmark

`materialize-provider-hierarchy` 会把 answer-free fine hierarchy input 与每个 sample 的一个
normalized provider block 文件连接起来。它先完成并写入所有 adapted input，之后才解析任何
source label path。Normalizer 会丢弃 provider sequence/relation 字段；input invariance 测试会
分别改变这些字段和 hierarchy label。存在 Paddle corpus-run manifest 时，其上游 corpus SHA 和
完整 generated + skipped sample-id 集合必须与 hierarchy source manifest 一致。

`benchmark-provider-hierarchy` 同样会先预测选中 partition 的所有 input，再打开 label。
`--partition` 强制 fit/calibration 隔离；每个 input、label、proposal、provider output、corpus
manifest、可选 provider-run manifest 与可选 relation model 都绑定 hash。冻结的 PP-DocLayout
membership 参数为 coverage `0.10`、margin `0.10`；带 padding 的 detector box 不直接复用 oracle
默认值 `0.80`。
...
提高到 `0.97433893`，flat 为 `0.94768195`。14 页 calibration 对应为
`0.93254330 -> 0.96754386`，flat 为 `0.97694650`。独立 32 页 provider test 的
precision/recall/F1 为 `0.96979086/0.97054264/0.97016660`，超过 flat `0.96606248`；
assignment coverage 为 `0.97687225`，segmentation pair F1 为 `0.80643143`。Semantic 与 native
v5 在该 test 上完全相同。由于 calibration 仍低于 flat 且 segmentation 仍不理想，报告决策保持
`provider-derived-development-benchmark-review-only`，runtime reorder 继续关闭。

Policy v6 会显式记录 provider origin，不再从 schema 名称猜测。真实 `DocumentIR` adapter 与
corpus materializer 都写入
`input_adapter.coarse_region_source = normalized-provider-structure`；旧 hierarchy input 或 oracle
input 不会启用新 guard。每个 provider region 中在 selected order 上连续的 member 保持同一
stream；被其他元素打断的 member pair 只有在归一化纵向 gap 位于平均行高
`[-0.25, 1.25]` 时才继续连接。否则该 region 会表示为带稳定 `-segment-NNN` id 的多个 stream，
不跨 discontinuity 构造 successor edge。

Native boundary transition 还需要 absolute selected-base rank displacement `<= 4`。被拒 edge
继续留在 `cross_region_relation_evidence`，suppression reason 为
`provider-nonlocal-selected-rank-gap`；外部 semantic edge 不受这个 native guard 影响。Stream
分段可能为 v5 unassigned fallback 释放 element degree slot，但 fallback 仍要求至少一个 endpoint
未分配，并保持 degree-one acyclic。Candidate order 在 fallback 层前计算，始终不变。

新增 diagnostics 包括 `provider_local_stream_split_count`、backward/gap discontinuity count 和
`fine_relation_provider_nonlocal_suppressed_count`。Fit/calibration/test provider relation F1 从
`0.97433893/0.96754386/0.97016660` 提高到
`0.97688390/0.97363796/0.97527740`。Test precision/recall 为
`0.97966367/0.97093023`，flat test F1 为 `0.96606248`。Oracle test control 的 line/region F1
逐项保持 `0.95571096/0.93055556`，provider-only action 为 0。14 页 calibration 仍低于 flat
`0.97694650`，因此 gate 继续保持 review-only。

相同 input 的 v5/v6 真实复杂页 replay 中，Attention PP 与 PUMA PP 没有触发；比亚迪 PP 抑制
1 条 nonlocal relation transition；JD/Docling 拆分 8 条 provider chain、抑制 6 条 nonlocal
native candidate，local edge 从 `89` 变为 `81`，relation transition 从 `11` 变为 `5`，带 guard
的 unassigned fallback transition 从 `10` 变为 `18`。这些是无标签 diagnostics，不是 accuracy
结论。四个页面的 base/candidate order 完全一致，runtime reorder 继续为 false。

Policy v7 会通过 `RelationGraphOrderEvidence.candidate_edges` 暴露 relation graph 在 selection
前的 sparse candidate。`_infer_relation_graph_result()` 原本就会构建该 graph，因此 hierarchy
直接复用同一次 pass，不会重新计算 column 和全部 pair score。Candidate record 只包含 source
index、target index 和 geometry score；答案字段不会进入契约。旧调用方仍可只构造两个字段，
因为 candidate tuple 默认为空。

Provider adjacency rescue 只检查属于不同 `text` region、尚未连接的连续 selected-native id。
Raw relation candidate score 必须 `>= 0.95`，normalized vertical gap 必须位于 v6 范围，水平
overlap 必须 `>= 0.5`。Element 与 region 的 source/target degree slot 都必须空闲，而且两个 graph
都不能成环。外部 semantic edge 保持独立。发出的 edge 使用
`provider-native-adjacency-relation-rescue` reason，confidence 上限 `0.76`，并保留 raw candidate
score、threshold、geometry、stream id 与 review-only provenance。被拒的 supported candidate
也会保留为 evidence，并记录 degree/cycle reason。

更宽的 geometry-only control 在独立窗口 precision 降到 `1/3`，因此被否决。Strict score bucket
在 fit、calibration、独立 test 的 selected-native adjacency 上分别为 `460/469`、`94/96`、
`318/321`；实际 rescue emission 分别为 `3/3`、`5/5`、`1/1`。Fit/calibration/test F1 达到
`0.97728460/0.97588777/0.97547684`。Diagnostics 会记录 considered、score-supported、
...
生成 review-only proposal，无需 labels。加载时会校验 manifest 中的 threshold、
feature count、estimator 类型/参数、nearest-candidate 设置和 model filename
是否与已绑定 hash 的 joblib bundle 一致，描述信息不能再与实际预测配置静默分叉。
`export-hierarchy-input` 从 DocumentIR
...
corpus/label schema，input 中仍不含 provider sequence/relation。Paragraph / successor
proposal schema v2 带有 `prediction_provenance`：fit proposal 必须声明 document-OOF
打分，evaluation proposal 声明冻结的 all-fit model，序列化 operator prediction 使用
独立 mode 并绑定 model/manifest hash。每条 benchmark proposal 还会记录 input、source
corpus 和 train corpus hash。Joint decode 会在 label 打开前拒绝 partition、producer、
mode、feature、input、corpus 或 cross-validation provenance 不一致的输入。
当前 paragraph 与 successor feature id 已升级为 v2：原始 v1 freeze 之后的两次共享
geometry / reading-order 变更改变了 candidate set，因此当前 predictor 不再接受 v1 模型。

Joint decode 优先打包合法 successor path cover 与 paragraph hierarchy 标签；当
...
输出保持 `runtime_reorder: false`，且不包含 oracle membership/scope 字段。Joint
proposal 会绑定两个 source proposal 的 hash。普通模式的 paragraph component 保留
`fine-line-paragraph-graph` 来源；successor-chain / geometry-chain fallback component
会标记真实来源、fallback reason 和原 paragraph component id。合成
多栏 fixture 在 `tests/test_joint_graph_benchmark.py` 覆盖该解码契约，模型
round-trip 在 `tests/test_graph_model.py`；完整 Comp-HRDoc 分区指标只在冻结语料
端到端重跑后记录。
...
- Sparse graph segmentation 与 cluster-and-sort reading order：https://arxiv.org/abs/2305.02577
- 带 degree/cycle constraint 的 multi-modal relation prediction：https://doi.org/10.1016/j.patcog.2024.110314
...
`RelationGraphOrderEvidence`，其中同时包含序列化 candidate index、selection 前的 sparse
candidate edge，以及每条进入 max-regret path cover 的 edge 在选择时的 diagnostics。Hierarchy
需要三种视图中的任意组合时都不必重复构造二次复杂度 candidate graph；原有 order-only 与
selected-edge API 都委托给同一 primitive。
...
在 semantic arbitration 和 v4 object-branch contract 之前，64 页 train-only v3 control 的
membership 为 `5244/5257 = 0.99752711`，错误分配为 0、unassigned 为 13；within-region F1
为 `0.99297033`，fit/calibration 分别为 `0.99191794/0.99642675`。Line cross-region F1
为 `0.93473962`，region-transition F1 为 `0.90607029`。Calibration line/region F1 为
`0.92260062/0.89759036`，其中 line 仍低于 flat `0.92879257`。v3 报告汇总 972 条
...
replay 中都触发 0 次。这些数值继续作为明确的 pre-semantic/pre-object-branch control；当前 v4
结果记录在上面的 semantic hierarchy 小节。
