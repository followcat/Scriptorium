...
- `column-flow-v1`: 轻量多栏回退，检测重复的两栏或三栏文本 flow，表格保持 row-major，并按栏再按纵向位置排序每个 flow segment。表格网格守卫现在还要求多单元行行高一致：数据表格的一行是一条记录、单元同高；杂志/索引页在同一视觉行混合标题行高与描述行高的单元，因此保持按栏读取。由 `tests/test_reading_order.py::test_toc_index_page_with_badges_stays_column_readable` 与 `test_uniform_narrow_cell_grid_stays_row_major` 覆盖，并在带标签的 Hello World 第 22 期目录页上把 successor accuracy 从 `0.21739130` 提高到 `0.60869565`。
