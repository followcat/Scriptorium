...
- PaddleOCR-VL / PP-StructureV3 / Docling JSON 可以通过 `--structure-json` 融合进 native IR，作为 role/order/table/formula 证据；显式 block order、Docling `body.children`、结构化 parsing-list 位置和嵌套子 block 顺序都可以在 bbox 匹配后成为 `external_structure_order`，Docling `furniture.children` 会提供 page-artifact 角色/stream 证据但不成为正文顺序证据，无序 `layout_det_res.boxes` 只作为 label/region 证据。结构 JSON 里的 `successor_edges`、`precedence_edges`、`relations`、`reading_streams` 现在也可以解析到已匹配元素，并优先形成安全的 path-cover 运行时重排；没有关系边时再回退到 block order。`reading_streams` / `streams` 也会写入面向翻译的 `reading_order_stream_*` 元数据，因此 structure-only sidecar 即使没有 region bbox，也能把 OCR/image 文本分成正文、边栏、表格、卡片/网格等局部流。更具体的子区域会在同分时优先于父区域，因此匹配到的模型 label 可以驱动 page-artifact、footnote、sidebar、caption、table-island，以及明确 card/grid/product/tile 类区域的 `grid-island` reading streams。普通 list label 只作为列表证据，不作为卡片网格证据。
