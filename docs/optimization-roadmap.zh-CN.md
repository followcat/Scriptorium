...
- Comp-HRDoc 图形 node 和 floating label 现已保留。Train-only floating gate 与共享 body/floating path cover 把 clean selected-edge F1 从 `0.86904643` 提高到 `0.88774602`，并在确定性噪声下保持增益。全局一对一 structure-role assignment 将启发式 graphical F1 从 `0.85631350` 提高到 `0.86868687`；provider text 保持 many-to-one，figure/table anchor 则全局唯一。在主要为单栏的固定 5 页 prefix 上，PaddleOCR-VL/Docling relation F1 为 `0.97183099/0.95260663`；固定复杂双栏 `1412.1395` p. 4 的 raw F1 仍为 `0.75609756/0.71604938`。固定 unified annotation 与 HDSA tree 在该页交叉绑定了两张图和 caption，因此新审计报告 2/2 条官方 graphical-label conflict，不再把它误判为重复 anchor assignment。Raw label 绝不改写，trained edge 仍未通过 strict gate，runtime 保持关闭。下一步用 train-only 数据校准剩余 caption-locality gate，并扩大经过 label audit 的复杂页 provider suite。
...
6. Relation graph 和 successor consensus 仲裁
   ...
   Figure/table-caption association 现使用 cardinality-first 最大权重二分 assignment，不再按输入顺序贪心抢占。固定且未改动的 250 页 graphical test corpus 上，启发式 graphical F1 为 `0.85631350 -> 0.86868687`，整体 role-fusion F1 为 `0.85716953 -> 0.85772965`，joint path-cover F1 为 `0.88501708 -> 0.88543574`。其中 1 个变化页因既有 25% 水平重叠门槛排除真实 caption 而回退。下一轮门槛必须仅从官方 train 文档选择和校准，再对固定 test corpus 运行一次；不能由已观察到的 test 页选择阈值。
