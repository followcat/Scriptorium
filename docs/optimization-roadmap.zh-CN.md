...
- Comp-HRDoc 图形 node 和 floating label 现已保留。固定 annotation-only benchmark 使用 250 页、53 个 test 文档和 347 条图形 label。Train-only floating-pair gate 把整体 raw F1 从 native `0.84129386` 提高到 `0.85943625`；图形 precision/recall/F1 为 `0.90168539/0.92507205/0.91322902`。由 calibration 产生的 `0.95` review gate 在 test 达到 precision `0.97647059`；再结合 fit-only feature envelope 后，在 28 个文档家族中得到 72/72。但 calibration 无法支持 `0.97` strict gate，因此 runtime promotion 仍关闭。下一步：提高 calibration support，并将高可靠 floating edge 与多栏 body path cover 联合评分，但不把 test 72/72 当作调参目标。
