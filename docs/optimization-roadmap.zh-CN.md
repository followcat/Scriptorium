...
- 来源无关的 fine-line paragraph graph 现在可以在不读取 provider rectangle 的前提下预测 co-membership。Fit-only 阈值 `0.94971959` 下，fit OOF/calibration/独立 test pair F1 为 `0.81549627/0.83054081/0.85162046`，selected-edge precision 为 `0.99202393/0.99642147/0.99548736`，相对 assigned-stream grouping 提高 `+0.09561271/+0.20843413/+0.05119419`。反转全部 160 页的 input element array 后 candidate 差异为 0，evaluation label 打开前已写完 proposal。它继续保持 benchmark-only，等待联合解码和跨领域标签。
- 独立的来源无关 directed successor head 现在达到 fit OOF/calibration/独立 test relation F1 `0.98391591/0.98679345/0.98585545`，稳定超过 provider hierarchy `0.97747518/0.97717622/0.97547684`。它不读取 provider region 或 paragraph label，保持输入 array 不变量，并通过 degree-one/cycle decoding 提高独立 top-one F1。完整 411 项测试通过。两个 head 联合解码、以流式/序列化方式限制 runtime 内存，并在年报、门户、中文文档和 image OCR 上验证前，它继续保持 benchmark-only。
