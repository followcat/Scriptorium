...
6. Relation graph 和 successor consensus 仲裁
   ...
   下一步要通过通用 relation evidence 补齐 calibration line endpoint 的剩余差距，不能对具体页面调 threshold。优先级是 provider 提供的局部 relation/stream edge、caption/figure 与 table boundary relation，以及按 document-level fold 校准的 abstention policy。全量 non-boundary stream split 与 relation-plus-flat expansion 已被否决，因为它们分别损伤 within continuity 和 region-transition F1。下一条 policy 必须在打开新文档族前冻结。Translation reflow 应消费已接受的 local stream 和 partial handoff，不能为了简化 replacement 而把 DAG 压平成 page-wide sequence。
