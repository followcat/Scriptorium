...
6. Relation graph 和 successor consensus 仲裁
   ...
   首轮答案隔离的 64 页 Comp-HRDoc train 结果是正向的。一次非迭代的四邻居 continuity 规则解除 8 个 exact geometry-tied membership，且全部正确，使 membership recall 从 `0.99353243` 提高到 `0.99505421`，within-region F1 从 `0.98901099` 提高到 `0.99188544`，同时没有改变 cross-region prediction。Line cross-region F1 从 `0.76518219` 提高到 `0.92624585`，region-transition F1 从 `0.72936660` 提高到 `0.89761751`；graphical-multicolumn 的 line/region F1 达到 `0.90066225/0.84472050`。这还不是 runtime promotion：calibration line F1 `0.90220820` 仍低于 flat control `0.92879257`，calibration region F1 `0.87730061` 仍低于 `0.88563050`，而且没有打开新的官方 test window。Membership coverage 也仍只证明 assignment，不证明 order correctness。

   下一步要通过通用 relation evidence 提升 calibration 与 graphical stratum，不能对具体页面调 threshold。优先级是 provider 提供的局部 relation/stream edge、caption/figure 与 table boundary relation，以及按 document-level fold 校准的 abstention policy。所有变化必须在打开新文档族前冻结。Translation reflow 应消费已接受的 local stream 和 partial handoff，不能为了简化 replacement 而把 DAG 压平成 page-wide sequence。
