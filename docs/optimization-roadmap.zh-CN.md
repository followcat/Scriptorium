...
- Comp-HRDoc 图形 node 和 floating label 现已保留。Train-only floating gate 与共享 body/floating path cover 把 clean selected-edge F1 从 `0.86904643` 提高到 `0.88774602`。固定确定性 source perturbation 现在可以在不根据结果调参的前提下检验 layout sensitivity：trained joint F1 在 mild（`0.85760048` vs. `0.84100588`）和 stress（`0.61358904` vs. `0.59914677`）下均高于 native，zero-OOD protected edge 分别为 65/65 和 39/39。Stress fragmentation 仍导致绝对分数大幅下降。下一步：在固定渲染子集上用匹配的 Paddle/Docling/真实 OCR anchor 取代合成 perturbation，并在 runtime 试验前提高 calibration support。
