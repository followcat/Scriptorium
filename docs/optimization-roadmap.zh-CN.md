...
- 固定版本的 Chunkr Reading Order Bench OSS 开发语料新增 733 个跨领域页面、9,267 个专业排序 layout element，candidate input 不使用 annotation id。Selected-auto 的 micro pairwise 高于 visual Y/X（`0.87452713` vs `0.84918215`），但 exact page match 更低（`0.61255116` vs `0.66030014`），说明下一步需要 selective arbitration。稳定 support-2 precision/recall 为 `0.91591928/0.67014296`；加入 XY-cut 后为 `0.82393096/0.74958988`，独立确认其 coverage/precision 权衡。第一次运行还修复了 grid-island/sidebar 重复所有权。下一步在该 development corpus 上训练并按文档交叉验证 role-aware pairwise successor/ranking candidate，再原样评估答案隔离的 Comp-HRDoc/ROOR 和复杂年报/门户 sidecar。
