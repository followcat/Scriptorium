...
- Caption-flow 识别 `Figure/Fig./Table/Algorithm + number` caption，把栏内 caption 留在本栏，把跨 gutter caption 标成局部 flow break。
- Box-flow、relation-graph、successor-consensus 都作为候选诊断进入 benchmark；runtime 只启用了非常窄的 successor-consensus 仲裁。
- Semantic sidecar 能直接给 selected、visual-yx、box-flow、relation-graph、successor-consensus、external-structure 候选打分，并输出候选是否值得考虑接管。
...
2. 递归 XY-Cut 和局部结构 refinement
   ...
   已实现 column-flow、table island、sidebar、footnote、caption、spatial graph、box-flow 和候选诊断。下一步重点是 caption-to-figure/table proximity、基于 sidecar 的置信度校准，以及 native heuristics、relation graph、successor consensus、role、table、caption、外部结构证据之间的 runtime arbitration。
...
6. Relation graph 和 successor consensus 仲裁
   ...
   当前 relation graph 已能降低若干复杂样本的 local successor disagreement，但 pairwise disagreement 仍高，不能直接默认接管。下一步应把 relation edges、candidate consensus、page-level recommendation、semantic sidecar、role/caption/table proximity 和外部模型证据结合起来，只在独立证据支持时切换顺序。
