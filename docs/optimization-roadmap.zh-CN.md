...
- Provider order 评测现在分开 block 内 line edge 与真正的 block transition，并正确处理 Comp-HRDoc partial labels。旧 `287/408` raw precision 与 `209/219` gate 值都把未标注边当错误，现仅保留为历史口径；同一已打开 test window 的 endpoint-aware gate 为 `256/268 = 0.95522388`，另有 16 条 unscored，但页首/页末 Wilson 仍失败。64 页 train-only gate v3 预声明 `support >= 2`，按 25 个 fit document 做 5-fold OOF：aggregate `192/195 = 0.98461538`，但两个 fold、`graphical-multicolumn/middle` 样本量和 `multicolumn/start` 质量未过线；7-document calibration 只有 `21/21`，Wilson `0.84536098` 且低于 30 条。Unanimous support 3 又无合格 fit bucket。因此保持 `document-cross-validation-rejected-review-only`，不打开新 test window。下一步应增加独立结构证据，而不是继续收紧阈值；优先评估 recursive XY-cut region tree 与 relation-graph selected edges，避免把 path-head serialization 当作直接 successor 证据。
