...
- Comp-HRDoc 图形 node 和 floating label 现已保留。Train-only floating gate 与共享 body/floating path cover 把 clean selected-edge F1 从 `0.86904643` 提高到 `0.88774602`，并在确定性噪声下保持增益。全局一对一 structure-role assignment 加 train-only caption locality calibration 将启发式 graphical F1 从 `0.85631350` 提高到 `0.87804878`；provider text 保持 many-to-one，figure/table anchor 则全局唯一。在主要为单栏的固定 5 页 prefix 上，PaddleOCR-VL/Docling relation F1 为 `0.97183099/0.95260663`；固定复杂双栏 `1412.1395` p. 4 的 raw F1 仍为 `0.75609756/0.71604938`。固定 unified annotation 与 HDSA tree 在该页交叉绑定了两张图和 caption，因此审计报告 2/2 条官方 graphical-label conflict，不再把它误判为重复 anchor assignment。Raw label 绝不改写，trained edge 仍未通过 strict gate，runtime 保持关闭。下一步扩大经过 label audit 的复杂页 provider suite，并为剩余歧义 caption 候选增加 train-calibrated semantic/OOD 证据。
...
6. Relation graph 和 successor consensus 仲裁
   ...
   Figure/table-caption association 现使用 cardinality-first 最大权重二分 assignment，不再按输入顺序贪心抢占。Caption locality 随后只用官方 train 文档校准：纵向 gap 保持 `0.12` 页高，取消强制水平 overlap，并把水平中心距离从 `0.50` 收紧到 `0.35` 页宽。Fit correct/predicted 从 `5284/5658` 提高到 `5291/5665`，document-hash calibration 从 `1348/1473` 提高到 `1349/1474`，fit/calibration 均无回退页。固定且未改动的 250 页 graphical test corpus 上，启发式 graphical F1 为 `0.85631350 -> 0.87804878`，整体 role-fusion F1 为 `0.85716953 -> 0.85803621`，joint path-cover F1 为 `0.88501708 -> 0.88575528`。下一步是更丰富的 train-calibrated caption semantic 与 OOD rejection，不再根据 test 调几何阈值。
