...
- Provider order 评测现在将 block 内的局部几何 line edge 与真正的模型 block direct transition 分开。32 个 train-only 页上，PP-DocLayoutV3 aggregate F1 为 `0.88870500`，direct transition precision 只有 `287/408 = 0.70343137`。只在 fit 冻结的 answer-free native-support + detector-confidence gate，在独立 32 页官方 test 上达到 `209/219 = 0.95433790`；但 graphical-multicolumn 未通过 Wilson 标准，multicolumn 未通过 point precision，页首/页末也弱于页中。因此 suite v5 以 veto-only 方式写入 `reject-runtime-promotion`；下一步必须先在 train 冻结 layout/position-aware rejection，再用从未打开过的文档验证。
