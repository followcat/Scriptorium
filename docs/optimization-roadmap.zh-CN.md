...
4. Fidelity 模式的编辑 mask 和 replacement fitting
   ...
   当前 edited/translated 节点可以打印为局部白底 overlay。翻译应按 `data-scriptorium-translation-stream-id` 分批应用，让正文、边栏、表格岛和卡片网格岛可以独立替换。下一步需要 edit-aware compositor：根据 glyph extents 推导 mask padding、自动压缩/换行长翻译文本、检测 replacement 与相邻元素冲突，并同时支持 SVG/raster 背景。
