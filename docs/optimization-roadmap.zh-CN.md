...
6. Relation graph 和 successor consensus 仲裁
   ...
   Paddle/PP-Structure 的显式 block order 现在只会作为连续 primary-block review transition 进入 sidecar，并保留 provider/order/label/bbox/member provenance 与独立的 direct/path 指标。下一步架构工作是让 image benchmark 同时接受稳定 OCR/layout anchor 与独立 structure-provider payload，再用 held-out ROOR relation 评估这些 transition，且答案标签不得进入模型融合。
