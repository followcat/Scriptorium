...
- PaddleOCR-VL / PP-StructureV3 / Docling JSON 可以通过 `--structure-json` 融合进 native IR，作为 role/order/table/formula 证据；显式 block order、Docling `body.children` 和结构化 parsing-list 位置都可以在 bbox 匹配后成为 `external_structure_order`，但无序 `layout_det_res.boxes` 只作为 label/region 证据。匹配到的模型 label 也会驱动 page-artifact、footnote、sidebar、caption、table-island，以及明确 card/grid/product/tile 类区域的 `grid-island` reading streams。普通 list label 只作为列表证据，不作为卡片网格证据。
