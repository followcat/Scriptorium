...
- `protected_successor_consensus` 现在用于验证下一层 relation，但不改变 runtime order：有效的原生 table/grid strict edge 会在通用 candidate edge 之前作为 hard local constraint 进入 acyclic path cover。它报告 protected 与 unresolved edge、明确的拒绝原因，以及约束序列化后仍缺失的严格边。在拥有 held-out relation-sidecar 覆盖前，它只用于诊断，并被排除在自动 candidate 建议之外。
...
- 下一轮 constrained-consensus 应把严格的 `table-local-order` / `grid-local-order` 作为 page-local hard constraint，而不能当成额外 selected-order 投票。它必须分别报告受保护和仍未解决的 successor edge，保持 island 边界，并且在 held-out relation-style sidecar 上证明收益前只作为诊断。
