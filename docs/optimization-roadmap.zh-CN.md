...
- Comp-HRDoc 图形 node 和 floating label 现已保留。固定 annotation-only benchmark 使用 250 页、53 个 test 文档和 347 条图形 label。Train-only floating-pair gate 把 raw F1 从 native `0.84129386` 提高到 `0.85943625`；其 high-precision fit-envelope 子集为 72/72，但由于 calibration 无法支持 `0.97` strict gate，仍为 review-only。共享 degree-one acyclic body/floating path cover 现在把 selected-edge F1 从 native `0.86904643` 提高到 `0.88774602`，保护全部 72 条可靠 float edge，并将 cycle rejection 从 18 降到 2。下一步：在 OCR/layout detection noise 下评估同一联合 selector，并在任何 runtime 试验前提高 calibration support。
