...
- Provider-derived hierarchy gate 现在已经可执行并保持答案隔离。PP-DocLayout block 替换 oracle region；provider sequence/relation 字段会被移除，run/corpus hash 必须一致，全部 input 在 label 打开前完成 prediction，partition filter 隔离 fit/calibration。Policy v5 仅在 membership abstention endpoint 上、通过 degree-one/acyclic guard 后保留 native adjacency。50 页 fit relation F1 从 `0.94842599` 提高到 `0.97433893`，14 页 calibration 从 `0.93254330` 提高到 `0.96754386`；独立 32 页 provider test 达到 `0.97016660`，超过 flat `0.96606248`。但 calibration flat 仍为 `0.97694650`，其 provider segmentation pair F1 只有 `0.65042468`，semantic reranking 在独立 test 上也没有新增收益。因此 runtime 继续关闭。下一步提高 provider grouping，或训练不读取答案的 hierarchy-vs-flat selector，然后重放年报与门户 relation sidecar。
- Provider continuity v6 会在 selected-order skip 缺少局部纵向连续性时拆分 detector region，并把跨越 4 个以上 selected rank 的 native relation jump 保持为 evidence-only。Provider adapter 显式写入 marker，外部 semantic edge 豁免，oracle input 完全不变。Fit/calibration/test F1 从 `0.97433893/0.96754386/0.97016660` 提高到 `0.97688390/0.97363796/0.97527740`；独立 test 比 flat 高 `0.00921492`，oracle line/region F1 逐项保持 `0.95571096/0.93055556`。相同 input 的真实重放在 JD/Docling 上触发 8 个 stream split 与 6 个 nonlocal abstention，在比亚迪 PP 上触发 1 个 abstention，在 Attention/PUMA PP 上不触发；这些只是无标签 diagnostics，base/candidate order 不变。Calibration 仍比 flat 低 `0.00330854`，因此 runtime 继续关闭，provider grouping 或不读取答案的 hierarchy/flat selector 仍是下一道 gate。
- Graph-supported adjacency v7 会复用 relation graph 在 selection 前的 sparse candidate，不重复构造 pair score。通用 geometry-only rescue 因独立 correctness 降到 `1/3` 而被否决；strict `score >= 0.95` bucket 在 fit/calibration/test 分别验证为 `460/469`、`94/96`、`318/321`。加入 text role、局部连续性、overlap、element/region degree 与 cycle guard 后，实际 rescue edge 分别为 `3/3`、`5/5`、`1/1` 正确，F1 达到 `0.97728460/0.97588777/0.97547684`；calibration 只比 flat 低 `0.00105873`，独立 test 仍高 `0.00941436`。Oracle 与四个真实复杂页 order 均不变，因此 runtime 继续关闭，后续转向 provider split/merge grouping，而不是扩大 flat fallback。
- Assigned-stream pair F1 现在会评价真正暴露给编辑和翻译流程的 grouping，并排除 `unassigned-fallback`。Fit/calibration/test 分别为 `0.67895654/0.65148234/0.80042627`，provider-region pair F1 对照为 `0.67995655/0.65042468/0.80643143`。由于 calibration 的微小提升在 fit 和独立 test 上退化，更广泛的 stream split 被否决；该指标保留为 grouping regression gate。
- Train-only 重建现在支持显式、可审计的整文档跳过，但默认仍为 fail-closed。真实运行在不放宽阈值的前提下重建 64 篇文档、128 页（fit/calibration 为 `102/26`），拒绝并补选两个 fit 文档；oracle membership/within/region-transition 分别为 `0.99786574/0.99211930/0.92806959`。后续研究转向分别预测 paragraph membership 与 successor 的 sparse line graph，而不是继续拆 provider rectangle。
- 128 页 PP-DocLayoutV3 重放使 provider relation F1 在 fit（`0.97747518 > 0.96136149`）与 calibration（`0.97717622 > 0.97578947`）均超过 flat，独立 test 也继续超过 flat（`0.97547684 > 0.96606248`）。Grouping 仍弱：assigned-stream pair F1 为 `0.71988356/0.62210668/0.80042627`，test 仍低于 provider-region grouping。重放还暴露并修复了无界 spatial predecessor cycle；隔离页面现在 `2.48s` 完成，当前 411 项测试通过，既有 benchmark 值不变。
- 扩大后的 strict safe-merge ranking 继续被否决。Document-OOF fit、calibration、独立 test AUC 为 `0.86211189/0.78760353/0.88807583`，但 fit-only 没有阈值能在至少 20 个 candidate 时达到 precision `>= 0.98`，最佳合格 bucket 也只有 `19/25`。正确的局部 successor edge 不足以证明完整 provider region 可以合并，下一种模型必须直接预测 line-level paragraph membership。
- 最初的来源无关 fine-line paragraph graph 可以在不读取 provider rectangle 的前提下预测 co-membership。其归档 feature-v1 运行使用 fit-only 阈值 `0.94971959`，fit OOF/calibration/独立 test pair F1 为 `0.81549627/0.83054081/0.85162046`，selected-edge precision 为 `0.99202393/0.99642147/0.99548736`，相对 assigned-stream grouping 提高 `+0.09561271/+0.20843413/+0.05119419`。反转全部 160 页的 input element array 后 candidate 差异为 0，evaluation label 打开前已写完 proposal。它继续保持 benchmark-only，等待跨领域标签与 runtime 晋升门槛。
- 最初的来源无关 directed successor head 达到 fit OOF/calibration/独立 test relation F1 `0.98391591/0.98679345/0.98585545`，稳定超过 provider hierarchy `0.97747518/0.97717622/0.97547684`。它不读取 provider region 或 paragraph label，保持输入 array 不变量，并通过 degree-one/cycle decoding 提高独立 top-one F1。可选 `--model-output` 会写出带哈希校验的 `.joblib` 模型；evaluation 按页批打分；`predict-*-graph` 可对 hierarchy input 生成单页 review proposal；`export-hierarchy-input` 可从 DocumentIR 导出 fine-only hierarchy input，无需 provider structure。它继续保持 benchmark-only，等待跨领域验证。
- Paragraph / successor proposal v2 把 benchmark provenance 从描述变成可执行约束：fit 必须使用 document-OOF mode，calibration/test 必须使用冻结的 all-fit model，serialized-model prediction 不能进入 fit 指标。Input/train/source corpus hash 和两个 source proposal hash 会一直保留到 joint 输出。Successor 派生的 paragraph packaging 也会记录真实 fallback 来源。模型加载会拒绝 threshold、estimator metadata、nearest-candidate 设置或 filename 的 manifest/bundle 分歧。由于后续共享 geometry 变更改变了 feature-v1 candidate set，当前 graph feature 已升级为 v2，predictor 会拒绝归档 v1 模型。
- `benchmark-joint-graph` 现在会在不重训的前提下联合解码两个 review-only head：within-paragraph successor 作为 protected path-cover edge，cross-paragraph edge 必须是 tail→head 且按 score 排序，joint proposal 保持 `runtime_reorder: false`，合成多栏 fixture 覆盖 answer separation 与 degree-one 冲突。`materialize-graph-hierarchy` 可在无 layout provider 时把 hierarchy corpus 转成 graph head 语料；8 页 Comp-HRDoc smoke 在放宽 gate 下以 successor path-cover 打包 paragraph hierarchy，joint relation F1 为 `0.97297297/0.95681063`；32 页 train-only smoke 达到 joint/successor relation F1 `0.97468010/0.98010471`；64 页 train-only smoke 达到 `0.97869507/0.97864552`。通过程序校验的 128/32 feature-v2 重放达到 fit-OOF successor F1 `0.98531274` / paragraph pair F1 `0.80447743`，独立 test joint/successor relation F1 `0.98125509`，multicolumn F1 `0.98839739`，cross-region recall `0.93658537`（仍 `runtime_reorder: false`；joint 在不损失 relation 的前提下打包 successor path cover）。年报/门户/中文文档/image-source 带标签验证与任何 runtime promotion 仍是下一道门槛。
- Successor topology v3 新增 20 个无答案的 column/flow/XY-region、mutual-rank、relative-gap 与 corridor-blocker feature（共 59 个），稳定 feature name 会进入 report 和 model manifest。在不变的冻结 128/32 协议上，fit-OOF/calibration/独立 test F1 从 `0.98531274/0.98773806/0.98125509` 提高到 `0.98619554/0.98798956/0.98247759`；graphical-multicolumn test F1 从 `0.97209302` 提高到 `0.97488372`，cross-region recall 从 `0.88571429` 提高到 `0.90000000`，paragraph segmentation 保持 `0.79045562`。稀疏零边页面现在会打包为 singleton stream。真实页 direct relation 有提升但 anchor path 结果混合，101-element candidate generation 每页增加约 `0.084 s`，仅 fit 的 rank-2/3 fallback 只增加 `0.00020753` test F1，因此 runtime 继续关闭。沿 ROOR/GraphDoc/XY-Cut++ 与 Hakim 等人的 max-regret path-cover 方向，这一结果促成了一项不强制全局总序的冻结 target-edge-theft 实验。
- `benchmark-successor-decoder-ab` 现在会用冻结 model/threshold 重放全部过阈值备选，并用动态 source-regret degree-one 无环 path cover 解码。Hash 和 exact-baseline 校验通过，但 calibration/独立 test 的 selected edge 与 F1 完全不变（`0.98798956` 和 `0.98247759`）：分别有 1,938/1,948 和 2,483/2,491 次决策只有一个可行 candidate。因此 max-regret 被否决为下一个质量杠杆。评测后 coverage 分析发现 2,453 条 test truth edge 中有 17 条不在 20-neighbor graph；nearest-80 使 candidate recall 达到 `1.0`，但 edge 从 54,764 增到 191,720。下一个冻结 feature version 应新增稀疏无答案 long-gap aligned/column-handoff candidate，不做暴力 neighbor 扩容；runtime 继续关闭。
...
1. 扩展复杂 PDF 的真实 semantic ground truth
   ...
   Attention、Transformer-XL、Hacker News 已有部分 sidecar。PUMA 年报和 JD 截图 PDF 还没有 semantic sidecar，却暴露了复杂图文混排、边栏、表格岛、OCR 网页顺序等问题。三个更难 source 已建立 baseline：Segment Anything（跨栏整宽浮动体密集的 CVPR 双栏）、Mamba（公式/算法打断双栏）、Hello World Magazine #22（candidate 分歧最高的真·三栏杂志）。下一步应覆盖年报、手册、表格、公式、脚注、附录、浮动体密集双栏、三栏杂志和更多网页打印 PDF。对于存在多种合理全局序列的页面，优先使用 `successor_edges`、`ro_linkings`、`reading_order_*` 和 `precedence_edges` 标注局部阅读关系；如果标签来自 OCR/结构 JSON，优先保留 `document` segment id，让评测侧按 id 解析到文本。
...
6. Relation graph 和 successor consensus 仲裁
   ...
   Chunkr ranker 给出第二个警告：同一个 flat permutation model 可以在 coarse mixed-role block 上得分很好，却破坏 fine line continuity。Hierarchy 层现通过显式 provider parent id 或 answer-free 文本/containment evidence 形成 coarse region，保留 selected local line order，并把 cross-region handoff 表达成 partial relation DAG，而不是一个 coarse permutation。Fine relation-graph edge 只有对齐 local stream 边界时才成为 transition；degree conflict、region cycle、non-boundary edge、tie 与 abstention 都保留为显式诊断。图表对象现在保持 branch 语义：figure 是 branch root，table 是 branch terminal，因此 object/caption 单元不会成为两个无关 body stream 之间的 through-path bridge。Candidate expansion 仍要求完整且精确的 chain 与完整 fine membership。

   答案隔离的 64 页 Comp-HRDoc train 结果是正向的。一次非迭代的四邻居 continuity 规则解除 8 个 exact geometry-tied membership；另一个独立 boundary 分支要求 relation/base 的 `A -> element -> B` 一致，并且原 tie 内只有一个 region 包含该文本，又解除 13 个。21 次 repair 全部正确，使 membership recall 从 `0.99353243` 提高到 `0.99752711`，within-region F1 从 `0.98901099` 提高到 `0.99297033`。随后 semantic conflict arbitration 每次只替换一条冲突 boundary edge，object-branch contract 则在不降低正确边召回的前提下去除 table-source/figure-target through path。当前 64 页 line/region F1 为 `0.94870087/0.92708333`，calibration 达到 `0.94968553/0.92638037`。规则冻结后，独立 32 页官方 test window 达到 `0.94811321/0.93055556`，超过 flat control `0.94712644/0.90064795`。Runtime 继续关闭，因为 oracle-region relation accuracy 不是 provider-derived region 的端到端证据，而且 membership coverage 仍只证明 assignment，不证明 order correctness。

   下一步要通过通用 relation evidence 补齐 calibration line endpoint 的剩余差距，不能对具体页面调 threshold。15 条 calibration 剩余 miss 中，8 条在 relation/base 都不存在、5 条只有 base、2 条是 membership gap，没有 exact non-boundary relation 命中。Fit-only 审计也否决了继续叠加 geometry vote：74 条“base boundary 但 selected relation evidence 不存在”的候选中只有 3 条 exact truth；visual Y/X、box-flow 与 recursive XY-Cut 三路共同支持的 4 条候选仍是 0 命中。因此优先级转为 provider 提供的局部 relation/stream edge、caption/figure 与 table boundary relation，或带 document-fold abstention 的独立 semantic successor scorer。全量 non-boundary stream split 与 relation-plus-flat expansion 继续被否决，因为它们分别损伤 within continuity 和 region-transition F1。下一条 policy 必须在打开新文档族前冻结。Translation reflow 应消费已接受的 local stream 和 partial handoff，不能为了简化 replacement 而把 DAG 压平成 page-wide sequence。
