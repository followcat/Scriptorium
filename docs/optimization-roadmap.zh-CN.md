...
- Comp-HRDoc 图形 node 和 floating label 现已保留。review-only structure-role geometry 层会输出可审计的 `figure -> caption` 和 `caption -> table` edge。在 `1401.3699` 前 5 页中，加入两条官方 figure-caption label 后，raw edge F1 从 `0.96226415` 提高到 `0.96713615`；下游顺序和视觉指标不变。在 runtime 提升前，仍需更广泛的 held-out table/figure 页验证。
