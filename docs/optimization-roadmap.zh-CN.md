...
- Comp-HRDoc 图形 node 和 floating label 现已保留。启发式 role fusion 与 learned floating decoder 都使用 cardinality-first 全局一对一 assignment；learned margin 同时考虑 source-row 和 target-column 竞争候选。Train-only calibration F1 从 `0.91295681` 提高到 `0.93560477`，未改动 graphical test F1 从 `0.91322902` 提高到 `0.91761364`，joint body/floating path-cover F1 从 `0.88774602` 提高到 `0.88839440`。Clean strict precision 为 `0.97512438`，但 mild/stress 降到 `0.96571429/0.93495935`，所以 runtime 继续关闭。Clean 的 5 条 strict 错误全部触及 label-audit conflict graphical，而 stress 的 8 条中只有 2 条，区分了 test-label 歧义与真实噪声错误。固定复杂页 `1412.1395` p. 4 上，PaddleOCR-VL/Docling raw F1 仍为 `0.75609756/0.71604938`；Paddle 的几何一致 edge 通过 strict + zero-OOD，但仍被交叉绑定的官方 label 判错。Raw label 绝不改写。下一步只用 train-derived perturbation 拟合独立的 noise-aware correctness/rejection 模型，并在任何 promotion 前联合评估 clean/noise risk-coverage。
...
6. Relation graph 和 successor consensus 仲裁
   ...
   Figure/table-caption association 现使用 cardinality-first 最大权重二分 assignment，不再按输入顺序贪心抢占。Caption locality 只用官方 train 文档校准：纵向 gap 保持 `0.12` 页高，取消强制水平 overlap，并把水平中心距离从 `0.50` 收紧到 `0.35` 页宽。Fit correct/predicted 从 `5284/5658` 提高到 `5291/5665`，document-hash calibration 从 `1348/1473` 提高到 `1349/1474`，fit/calibration 均无回退页。Learned floating decoder 现在在每个 calibration threshold 上执行相同的全局 assignment，并用 source/target 竞争候选最小 gap 作为 assignment margin。Calibration F1 提高到 `0.93560477`，固定 test graphical F1 为 `0.91761364`，clean strict precision 为 `0.97512438`。Mild/stress strict precision 仍低于 `0.97`；下一步是独立的 train-noise-aware correctness forecaster，输入 pair confidence、row/column margin、assignment stability、OOD feature 与 perturbation diagnostics，并只在 train-derived fit/calibration 上选择 clean/noise risk-coverage operating point，test 只用于最终评估。相关方法依据包括 assignment confidence（https://doi.org/10.1016/j.patrec.2015.07.010）、calibrated structured prediction（https://proceedings.neurips.cc/paper/2015/file/52d2752b150f9c35ccb6869cbf074e48-Paper.pdf）和 noise-aware selective calibration（https://arxiv.org/abs/2208.12084）。
