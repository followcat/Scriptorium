...
- Comp-HRDoc 图形 node 和 floating label 现已保留。review-only structure-role geometry 层会输出可审计的 `figure -> caption` 和 `caption -> table` edge，并通过不含答案的 block membership 支持多行 caption。固定 33 页、2 文档 prefix 包含 18 条图形 relation；role edge 为 18/18，整体 raw F1 从 `0.94804157` 提高到 `0.95552025`。官方 order id 仍只在 sidecar。在 runtime 提升前，仍需更大的跨文档 floating split 和校准的 rejection。
