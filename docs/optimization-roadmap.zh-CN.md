...
- Comp-HRDoc 图形 node 和 floating label 现已保留。Train-only floating gate 与共享 body/floating path cover 把 clean selected-edge F1 从 `0.86904643` 提高到 `0.88774602`，并在确定性噪声下保持增益。真实 provider matcher 现已实现，且不使用 oracle list order。在主要为单栏的固定 5 页 prefix 上，PaddleOCR-VL/Docling relation F1 为 `0.97183099/0.95260663`；在固定复杂双栏 `1412.1395` p. 4 上降到 `0.75609756/0.71604938`。两者都在低于 gate 的 confidence 下错配 float，并被可靠性策略拒绝。下一步：扩大固定复杂页 provider suite，结合 provider hierarchy 和局部 assignment 改进 figure/caption association，并在 calibration 支持 strict gate 前保持 runtime 关闭。
