...
- 项目自有的轻量 relation ranker 现在可以从 ROOR 官方 train split 可复现训练，包含 UID-hash calibration、答案 relation 输入拒绝、model manifest hash 和 isolated review-only 输出。49 个 held-out validation 页面上，直接预测边 precision/recall/F1 为 `0.68715305/0.66347626/0.67510713`；解码后的 external candidate 把官方 relation accuracy 从 native `1274/2612`（`0.48774885`）提高到 `1513/2612`（`0.57924962`），并在 34 页上最佳。它仍不进入 runtime；下一步聚焦校准分叉、跨文档族泛化，以及独立论文/门户 relation label。
