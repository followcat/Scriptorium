...
6. Relation graph 和 successor consensus 仲裁
   ...
   Chunkr ranker 给出第二个警告：同一个 flat permutation model 可以在 coarse mixed-role block 上得分很好，却破坏 fine line continuity。下一版 prototype 应采用分层且保留 provenance 的结构。第一层通过显式 provider parent id 或 answer-free containment/overlap assignment 形成 coarse region，歧义和未分配 line 不进入 learned path；第二层只用粒度一致的模型排序 coarse region；第三层按 native 或独立验证过的 line-level successor 展开每个 region，并把 cross-region transition 与 within-region edge 分开评分。只有在未打开语料的两个层级都提升才允许 promotion，aggregate pairwise 收益不能抵消 successor 回退。
