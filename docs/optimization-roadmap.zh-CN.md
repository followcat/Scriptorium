...
6. Relation graph 和 successor consensus 仲裁
   ...
   Reading stream 层借鉴 PDF article threads 的架构：复杂排版可以显式暴露穿过非连续区域的局部阅读路径。它也和 relation/path-cover 类阅读顺序方法一致，因为复杂页面里更稳定的信号往往是局部 successor / precedence 关系，而不是一个脆弱的全局序列。
