...
- 独立的来源无关 directed successor head 现在达到 fit OOF/calibration/独立 test relation F1 `0.98391591/0.98679345/0.98585545`，稳定超过 provider hierarchy `0.97747518/0.97717622/0.97547684`。它不读取 provider region 或 paragraph label，保持输入 array 不变量，并通过 degree-one/cycle decoding 提高独立 top-one F1。它继续保持 benchmark-only，等待序列化、有界内存训练和跨领域验证。
- `benchmark-joint-graph` 现在会在不重训的前提下联合解码两个 review-only head：within-paragraph successor 作为 protected path-cover edge，cross-paragraph edge 必须是 tail→head 且按 score 排序，joint proposal 保持 `runtime_reorder: false`，合成多栏 fixture 覆盖 answer separation 与 degree-one 冲突。完整 Comp-HRDoc joint 指标、模型序列化，以及年报/门户/中文文档/image-source 标签仍是下一道门槛。
