...
- 独立的来源无关 directed successor head 现在达到 fit OOF/calibration/独立 test relation F1 `0.98391591/0.98679345/0.98585545`，稳定超过 provider hierarchy `0.97747518/0.97717622/0.97547684`。它不读取 provider region 或 paragraph label，保持输入 array 不变量，并通过 degree-one/cycle decoding 提高独立 top-one F1。可选 `--model-output` 会写出带哈希校验的 `.joblib` 模型；evaluation 按页批打分；`predict-*-graph` 可对 hierarchy input 生成单页 review proposal。它继续保持 benchmark-only，等待跨领域验证。
