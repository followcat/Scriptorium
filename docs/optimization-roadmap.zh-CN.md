...
- 来源无关的 fine-line paragraph graph 现在可以在不读取 provider rectangle 的前提下预测 co-membership。Fit-only 阈值 `0.94971959` 下，fit OOF/calibration/独立 test pair F1 为 `0.81549627/0.83054081/0.85162046`，selected-edge precision 为 `0.99202393/0.99642147/0.99548736`，相对 assigned-stream grouping 提高 `+0.09561271/+0.20843413/+0.05119419`。反转全部 160 页的 input element array 后 candidate 差异为 0，evaluation label 打开前已写完 proposal。它继续保持 benchmark-only，等待跨领域标签与 runtime 晋升门槛。
- 独立的来源无关 directed successor head 现在达到 fit OOF/calibration/独立 test relation F1 `0.98391591/0.98679345/0.98585545`，稳定超过 provider hierarchy `0.97747518/0.97717622/0.97547684`。它不读取 provider region 或 paragraph label，保持输入 array 不变量，并通过 degree-one/cycle decoding 提高独立 top-one F1。可选 `--model-output` 会写出带哈希校验的 `.joblib` 模型；evaluation 按页批打分；`predict-*-graph` 可对 hierarchy input 生成单页 review proposal；`export-hierarchy-input` 可从 DocumentIR 导出 fine-only hierarchy input，无需 provider structure。它继续保持 benchmark-only，等待跨领域验证。
- `benchmark-joint-graph` 现在会在不重训的前提下联合解码两个 review-only head：within-paragraph successor 作为 protected path-cover edge，cross-paragraph edge 必须是 tail→head 且按 score 排序，joint proposal 保持 `runtime_reorder: false`，合成多栏 fixture 覆盖 answer separation 与 degree-one 冲突。`materialize-graph-hierarchy` 可在无 layout provider 时把 hierarchy corpus 转成 graph head 语料；8 页 Comp-HRDoc smoke 在放宽 gate 下以 successor path-cover 打包 paragraph hierarchy，joint relation F1 为 `0.97297297/0.95681063`；32 页 train-only smoke 达到 joint/successor relation F1 `0.97468010/0.98010471`；64 页 train-only smoke 达到 `0.97869507/0.97864552`；冻结 128/32 协议复现 fit-OOF successor F1 `0.98391591` / paragraph pair F1 `0.81549627`，独立 test joint/successor relation F1 `0.98287811`，multicolumn F1 `0.99019964`，cross-region recall `0.94146341`（仍 `runtime_reorder: false`；joint 在不损失 relation 的前提下打包 successor path cover）。年报/门户/中文文档/image-source 带标签验证与任何 runtime promotion 仍是下一道门槛。
