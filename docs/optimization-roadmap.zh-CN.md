...
- Comp-HRDoc 图形 node 和 floating label 现已保留。固定 annotation-only benchmark 使用 250 页、53 个 test 文档和 347 条图形 label。专用 floating-pair gate 现在只用官方 train member 训练，并按 document hash 隔离 fit/calibration。在未改动 test prefix 上，整体 raw F1 从 native `0.84129386` 提高到 `0.85943625`；图形 precision/recall/F1 为 `0.90168539/0.92507205/0.91322902`，高于启发式 fusion 的 `0.86257310/0.85014409/0.85631350`。它仍为 review-only。下一步：confidence reliability、文档家族 rejection，以及与多栏 body path cover 集成。
