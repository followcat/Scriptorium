...
7. 真实模型证据 A/B
   ...
   PaddleOCR-VL 1.6 现在已有真实 PUMA 第 5 页的重放路径。匹配 bbox 前会归一化它的 `width` / `height` 模型输入画布，包括嵌套 result wrapper，因此同一份 96-DPI raw JSON 在 96 和 144 DPI benchmark 渲染下都得到相同的 24 个结构匹配。该运行没有模型 relation 或 stream edge，所以 selected order 与 candidate diagnostics 保持 native baseline；它验证的是可移植的结构证据，不是虚构的顺序提升。下一类 provider 应提供局部 relation/stream 证据，或接入专门的 relation predictor 生成可审查 successor edge，然后再扩大运行时顺序仲裁。
