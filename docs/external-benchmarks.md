...
New benchmark reports also include `reading_order_stream_count`, `reading_order_stream_type_counts`, and `reading_order_stream_id_counts`. The next PUMA/JD refresh should use these fields to show how many local body/sidebar/footnote/artifact/table-island streams are exposed before sidecar labels or external structure evidence are added.
