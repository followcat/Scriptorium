...
- `fidelity` HTML mode renders the original page as an SVG background while keeping transparent editable text nodes aligned by PDF coordinates. During print, fidelity overlay nodes are hidden and only the SVG page background is printed. This raised strict source-preservation scores to `0.98809524` on Attention, `0.9750043` on Transformer-XL, and `0.99490923` on web-HN. It is an architecture path for future editing: edited/translated regions still need a mask and replacement layer before fidelity mode can print user edits.
