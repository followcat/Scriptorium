...
In `fidelity` mode, edited/translated replacements use `fidelity-replacement-fit-v2`. The exporter calculates local white-mask padding, then independently clamps each side at adjacent visible boxes while ignoring enclosing page/background containers. It records the applied padding in `data-scriptorium-replacement-mask-padding`, the constrained sides and ids in `data-scriptorium-replacement-padding-constrained`, `data-scriptorium-replacement-padding-constraint-ids`, and `data-scriptorium-replacement-padding-constraints`, keeps replacement text aligned to the original bbox with CSS padding, applies `data-scriptorium-replacement-fit-scale` for long text, and flags `data-scriptorium-replacement-overflow` / `data-scriptorium-replacement-conflict` with neighboring element ids when the replacement needs review. Padding constraints prevent collateral masking; they intentionally do not relax a text-fit overflow.
...
- An explicit ordered body/paragraph block can also become an `external-block-body-*` translation stream when at least two matched text elements have coverage of at least `0.5`, share one selected native flow segment and column, and are not already owned by an explicit/relation stream. The members retain their selected local order; this path never changes page order. Generic diagnostic-only blocks and table/grid/caption/artifact/footnote/sidebar members are excluded. `derived_block_stream_count`, `derived_block_stream_member_count`, and `derived_block_streams_by_page` make the conservative derivation inspectable.
