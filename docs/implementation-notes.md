...
- `column-flow-v1`: a lightweight multi-column fallback that detects repeated two- or three-column text flows, keeps tables row-major, and orders each flow segment by column then vertical position. The table-grid guard additionally requires multi-cell rows to be height-uniform: data-table rows are single records with same-height cells, while magazine/index pages mix title-height and description-height cells in one visual row and stay column-readable. Footnote detection rejects note clusters that continue multi-column body text at body height (real footnotes are smaller, or sit on single-column pages), and grid-island runs require height-uniform rows so bottom index entries do not become fake card grids. Recursive XY-cut yields to this stack only when a page has three balanced non-table text columns. Covered by `tests/test_reading_order.py` TOC/footnote/grid regression tests and by the labelled Hello World issue 22 contents page where successor accuracy improved `0.21739130 -> 0.78260870` and pair accuracy `0.74275362 -> 0.96014493`.
