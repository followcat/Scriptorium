...
<p align="center">
  ...
  ·
  <a href="https://followcat.github.io/Scriptorium/">Gallery Demo</a>
  ·
  <a href="#文档">文档</a>
</p>
...
| OCR/版面模型验证 | 接入 PaddleOCR-VL、PP-Structure、Docling、OpenDataLoader 和 ROOR 风格结构 JSON，让 OCR/结构 JSON 主导 image source 的语义层，并做 native-only 与 native-plus-structure A/B benchmark。 |
...
检测到的表格岛和卡片网格岛会把严格的原生局部 successor 证据与全页 candidate consensus 分开报告。诊断专用的 protected path-cover 候选会把有效 island 严格边作为约束，并单独报告拒绝或未解决的边。这样能稳定翻译流，同时不会假装正文或跨区域 handoff 已经解决。
...
<p align="center">
  <a href="https://followcat.github.io/Scriptorium/"><strong>打开独立 Gallery Demo</strong></a>
  ·
...
对于数字 PDF，还可以使用 Apache-2.0 的 OpenDataLoader XY-Cut++ CPU provider。
它需要 Java 11+，输出始终为 review-only，不改变 runtime order：
...
仍未全部通过，因此 runtime promotion 继续关闭，也没有打开新的 test window。
...
外部 OCR/结构模型不是核心依赖。没有 OCR/结构 JSON 时，image source 仍会得到整页图片 visual layer；有 OCR 或 Paddle/PP-Structure/Docling/ROOR 风格结构 JSON 时，会生成透明文本锚点和 reading-stream 证据。OpenDataLoader 本身只处理 PDF，不改变 image source 的一等输入地位。
