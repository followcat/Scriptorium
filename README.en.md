...
Fidelity replacements now export `data-scriptorium-replacement-policy`, `data-scriptorium-replacement-fit-scale`, `data-scriptorium-replacement-mask-padding`, `data-scriptorium-replacement-overflow`, `data-scriptorium-replacement-conflict`, and `data-scriptorium-replacement-conflict-ids`. The exporter expands the local white mask for edited/translated text, pads the text back to the original coordinate, shrinks long replacement text to fit the source bbox when possible, and marks replacements for review when the mask overlaps adjacent elements or still overflows.

Benchmarks now also report `fidelity_replacement_element_count`, `fidelity_replacement_overflow_count`, `fidelity_replacement_conflict_count`, `fidelity_replacement_conflict_target_count`, `fidelity_replacement_min_fit_scale`, `fidelity_replacement_mean_fit_scale`, and `fidelity_replacement_policy_counts`, giving translated documents a measurable replacement-risk signal beyond pixel similarity. Replacement risk is also grouped by reading stream id/type, so a complex page can show whether translated conflicts concentrate in body text, sidebars, table islands, or card grids.
